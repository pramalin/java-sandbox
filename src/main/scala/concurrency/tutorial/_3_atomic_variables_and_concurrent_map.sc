...
object _3_atomic_variables_and_concurrent_map {
  println("Welcome to the Scala worksheet")       //> Welcome to the Scala worksheet
	...
	Welcome to the third part of my tutorial series about multi-threaded programming in Java 8. This
  tutorial covers two important parts of the Concurrency API: Atomic Variables and Concurrent Maps.
  Both have been greatly improved with the introduction of lambda expressions and functional
  programming in the latest Java 8 release. All those new features are described with a bunch of easily
  understood code samples. Enjoy!
	...
	without using the synchronized keyword or locks as shown in my previous tutorial.
	...
	synchronizing via locks. So my advice is to prefer atomic classes over locks in case you just have to
	...
	in a thread-safe manor without synchronizing the access to the variable. The method incrementAndGet()
	is an atomic operation so we can safely call this method from multiple threads.
	
	AtomicInteger supports various kinds of atomic operations. The method updateAndGet() accepts
	a lambda expression in order to perform arbitrary arithmetic operations upon the integer:
